<?php 
  require_once __DIR__."/../widgets/header.php"; 
  require_once __DIR__."/../widgets/sidebar.php";
  //dd(pageCount('news'), true);
?>

 <!--begin::App Main-->
 <main class="app-main">
        <!--begin::App Content Header-->
        <div class="app-content-header">
          <!--begin::Container-->
          <div class="container-fluid">
            <!--begin::Row-->
            <div class="row">
              <div class="col-sm-6"><h3 class="mb-0">Yangiliklar</h3></div>
              <div class="col-sm-6">
                <ol class="breadcrumb float-sm-end">
                  <li class="breadcrumb-item"><a href="/admin">Asosiy</a></li>
                  <li class="breadcrumb-item active" aria-current="page">Yangliklar</li>
                </ol>
              </div>
              <div class="col-sm-12 d-flex justify-content-end">
                <a href="?acontroller=news_create" class="btn btn-success">+Qo'shish</a>
              </div>

              <?php if(isset($_SESSION['success']) && !empty($_SESSION['success'])) :?>
                <div class="col-sm-12 mt-2 success_alert">
                    <div class="alert alert-success"><?=$_SESSION['success']?></div>
                </div>
                <?php unset($_SESSION['success']); // xabarni chiqarilgandan keyin o‘chiradi ?>
              <?php endif; ?>


            </div>
            <!--end::Row-->
          </div>
          <!--end::Container-->
        </div>
        <!--end::App Content Header-->
        <!--begin::App Content-->
        <div class="app-content">
          <!--begin::Container-->
          <div class="container-fluid">
            <div class="card mb-4">
                  <div class="card-header"><h3 class="card-title">Yangliklar</h3></div>
                  <!-- /.card-header -->
                  <div class="card-body">
                    <table class="table table-bordered" role="table">
                      <thead>
                        <tr>
                          <th>#</th>
                          <th>ID</th>
                          <th>Title</th>
                          <th>Kategoriyasi</th>
                          <th>Ko'rishlar soni</th>
                          <th>Yaratilgan vaqti</th>
                          <th>Rasmi</th>
                          <th>Status</th>
                          <th>Amallar</th>
                        </tr>
                      </thead>
                      <tbody>
                        <?php $i=1; if(!empty($news)):?>
                            <?php foreach($news as $newsItem): 
                              $image=getImage('news', $newsItem['id'], $newsItem['img'])
                              
                              ?>
                              
                                <tr>
                                  <td><?=$i++?></td>
                                  <td><?=$newsItem['id']?></td>
                                  <td><?=$newsItem['title']?></td>
                                  <td><?=$newsItem['category_name']?></td>
                                  <td><?=$newsItem['seen_count']?></td>
                                  <td><?=date('Y.m.d | H:i', strtotime($newsItem['create_at']))?></td>
                                  <td><img src="<?=$image?>" alt="" style="width: 120px; height:100px; object-fit:cover" ></td>
                                  <td>
                                    <?php if($newsItem['status']==STATUS_ACTIVE): ?>
                                        <span class="badge text-light bg-success">Aktiv</span>
                                    <?php else:?>
                                      <span class="badge badge-danger text-light bg bg-danger">Aktiv emas</span>
                                    <?php endif?>
                                  </td>
                                  <td>
                                    <a href="?acontroller=news_update&id=<?=$newsItem['id']?>" class="btn btn-success">
                                      <i class="fas fa-pencil"></i>
                                    </a>
                                    <a data-action="news_delete" href="?acontroller=news_delete&id=<?=$newsItem['id']?>" class="btn btn-danger delete_btn" data-id="<?=$newsItem['id']?>">
                                      <i class="fas fa-trash"></i>
                                    </a>
                                  </td>
                                </tr>
                            <?php endforeach?>
                        <?php endif ?>
                      </tbody>
                    </table>
            </div>
                  <!-- /.card-body -->
                  <div class="card-footer clearfix">
                    <ul class="pagination pagination-sm m-0 float-end">
                      <li class="page-item"><a class="page-link" href="?acontroller=news_index&page=<?=$page-1?>">«</a></li>
                      <?php if(isset($page_count) && !empty($page_count)): ?>
                        <?php for($i=1; $i<=$page_count; $i++):?>
                          <li class="page-item"><a class="page-link" href="?acontroller=news_index&page=<?=$i?>"><?=$i?></a></li>
                        <?php endfor?>
                      <?php endif?>
                      <li class="page-item"><a class="page-link" href="?acontroller=news_index&page=<?=$page+1?>">»</a></li>
                    </ul>
                  </div>
                </div>
             </div>
          <!--end::Container-->
        </div>
        <!--end::App Content-->
      </main>
      <!--end::App Main-->
<?php
 require_once __DIR__."/../widgets/footer.php";
?>
